  ...
  for i in "${!SESSIONS[@]}"; do
    ...
    if [[ "$_cs_secs" -gt 0 && -n "$_lk" ]]; then
      ...
      if [[ $_next -le $_now ]]; then
        next_kick="${YLW}$(TZ=America/New_York date -d @$_next '+%b %-d %-I:%M %p')${RST}"
      else
        next_kick="$(TZ=America/New_York date -d @$_next '+%b %-d %-I:%M %p')"
      fi
...
cmd_status_json() {
  ...
  for i in "${!SESSIONS[@]}"; do
    ...
    if [[ "$_cs_secs" -gt 0 && -n "$_lk" ]]; then
      ...
      if [[ $_next -le $_now ]]; then
        ...
        nk="$(TZ=America/New_York date -d @$_abs_next '+%b %-d %-I:%M %p')"
      else
        nk="$(TZ=America/New_York date -d @$_next '+%b %-d %-I:%M %p')"
      fi
    ...
    if [[ -n "$_lk" && "$_lk" -gt 0 ]] 2>/dev/null; then
      lk_fmt="$(TZ=America/New_York date -d @$_lk '+%b %-d %-I:%M %p')"
    fi
  ...
  gov_next=$(systemctl list-timers kick-governor.timer --no-pager 2>/dev/null \
       | awk 'NR==2{print $1,$2,$3,$4}' \
       | xargs -I{} bash -c "TZ=\"$HIVE_TZ\" date -d \"{}\" \"+%b %-d %-I:%M %p %Z\"" 2>/dev/null || echo "")
